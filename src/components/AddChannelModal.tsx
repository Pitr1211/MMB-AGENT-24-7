...
interface AddChannelModalProps {
  onClose: () => void;
  onAdd: (channelId: string, autoSync: AutoSync, status: ChannelStatus) => Channel | null;
}
...
export default function AddChannelModal({ onClose, onAdd }: AddChannelModalProps) {
  ...
  function extractChannelId(input: string): string {
    ...
    // youtube.com/@handle or youtube.com/c/name — use as-is (simulated)
    const handleMatch = trimmed.match(/youtube\.com\/@([\w-]+)/);
    if (handleMatch) return 'UC' + handleMatch[1].slice(0, 22).padEnd(22, 'x');
    const cMatch = trimmed.match(/youtube\.com\/c\/([\w-]+)/);
    if (cMatch) return 'UC' + cMatch[1].slice(0, 22).padEnd(22, 'x');
    // Fallback: use input as channel ID
    return trimmed;
  }
  ...
  async function handleSubmit() {
    ...
    setFoundInfo(`Found: Channel_${channelId.slice(2, 7)} • 15-20 videos`);
    await delay(1000);
    ...
    // Step 4: Actually add
    const result = onAdd(channelId, autoSync, status);
    if (result) {
      setLoadingStep('done');
      await delay(500);
      onClose();
    } else {
      // Error (duplicate, etc.) — toast already shown by store
      setLoadingStep('idle');
    }
  }
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <div className="mt-2 text-gray-600 text-xs space-y-0.5">
              <p>Accepts:</p>
              <p className="pl-2">&bull; UCxxxxxxxxxxxxxxxxxx (ID)</p>
              <p className="pl-2">&bull; youtube.com/@handle</p>
              <p className="pl-2">&bull; youtube.com/c/name</p>
              <p className="pl-2">&bull; youtube.com/channel/UCxxxxxx</p>
            </div>
          </div>
