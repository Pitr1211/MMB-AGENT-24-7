...
export default function NewProfileModal({ onClose, onCreate }: NewProfileModalProps) {
  ...
  const OS_OPTIONS: { os: OS; icon: React.ReactNode; desc: string; color: string }[] = [
    {
      ...
      desc: 'Desktop fingerprint — Chrome on Windows 10/11. Best for YouTube watch time.',
      color: 'blue',
    },
    {
      os: 'Android',
      icon: <Smartphone size={28} />,
      desc: 'Mobile fingerprint — Random Android device (Samsung/Pixel/OnePlus). Natural mobile behavior.',
      color: 'green',
    },
    {
      os: 'macOS',
      icon: <Apple size={28} />,
      desc: 'macOS fingerprint — Chrome/Safari on MacBook. Premium profile appearance.',
      color: 'purple',
    },
  ];
  ...
  const handleCreate = () => {
    ...
    const stepsData = [
      ...
      { delay: 1800, msg: '📡  Step 4: MoreLogin API — POST /api/env/setProxy/batch...' },
      { delay: 2400, msg: '🚀  Step 5: startProfile(envId) — waiting for unique IP assignment...' },
      { delay: 3000, msg: '💾  Step 6: SQLite INSERT — profile_id, proxy_config, fingerprint saved' },
      { delay: 3500, msg: '✅  Profile created successfully! Unique IP + Fingerprint assigned.' },
    ];

    stepsData.forEach(({ delay, msg }) => {
      setTimeout(() => {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <div>
            ...
            <p className="text-gray-500 text-xs mt-0.5">
              {step === 1 ? 'Step 1 of 1 — Choose your OS (rest is automatic)' : 'Auto-creating profile...'}
            </p>
          </div>
          {!creating && (
          ...
          {!creating ? (
            <>
              ...
              <div className="mb-2">
                <p className="text-gray-400 text-sm mb-4 bg-gray-800/60 border border-gray-700 rounded-xl p-3">
                  ...
                  All proxy generation, fingerprinting, MoreLogin setup, and SQLite storage happens automatically.
                </p>
              </div>
            ...
            <>
              ...
              <div className="space-y-2">
                {stepLogs.map((log, i) => (
                  <div key={i} className="flex items-start gap-2 text-sm animate-pulse-once">
                    <span className="text-gray-300 font-mono leading-relaxed">{log}</span>
                  </div>
                ))}
                {stepLogs.length < 7 && (
