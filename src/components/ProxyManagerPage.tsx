...
export default function ProxyManagerPage({ profiles, onRenewProxy }: ProxyManagerPageProps) {
  ...
  const expiredProxies = profiles.filter(p => p.proxy.expiresAt < now);
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
          ...
          {profiles.length === 0 ? (
            ...
            <div className="space-y-2">
              {profiles.map(p => {
                const timeLeft = p.proxy.expiresAt - now;
                const isExpired = timeLeft <= 0;
                const pct = Math.max(0, Math.min(100, (timeLeft / (parseInt(p.proxy.life) * 3600000)) * 100));
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-gray-500 text-xs">{p.proxy.city}, {p.proxy.state}</span>
                      ...
                      <div className="text-right flex-shrink-0">
                        <div className={`text-xs font-medium ${isExpired ? 'text-red-400' : 'text-gray-300'}`}>
                          {formatTime(timeLeft)}
                        </div>
                        <div className="text-gray-600 text-xs">{p.proxy.life}</div>
                      </div>
