...
export function generateProxyConfig(life?: ProxyLife): ProxyConfig {
  ...
  const selectedLife = life || randomFrom([...PROXY_LIVES]);
  const now = Date.now();
  const username = `${PROXY_PREFIX}_area-US_state-${state}_city-${city}_life-${selectedLife}_session-${sessionId}`;
...
export function renewProxySession(existing: ProxyConfig): ProxyConfig {
  ...
  const now = Date.now();
  const username = `${PROXY_PREFIX}_area-US_state-${existing.state}_city-${existing.city}_life-${existing.life}_session-${newSessionId}`;
  return {
    ...existing,
    username,
